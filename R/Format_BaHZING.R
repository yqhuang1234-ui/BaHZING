...
#'   - `taxon_columns`: names of the outcome columns in `Table`.
...
Format_BaHZING <- function(phyloseq.object, taxa_levels = default_taxa_levels) {
  ...
  if ("Domain" %in% colnames(taxa.table) &&
      !("Kingdom" %in% colnames(taxa.table))) {
    # Treat Domain as Kingdom when a Kingdom column is not already present.
    taxa.table <- taxa.table %>%
  ...
  # If kingdom information is present, reject missing values explicitly.
  if ("Kingdom" %in% colnames(taxa.table) &&
      any(is.na(taxa.table$Kingdom) | taxa.table$Kingdom == "k__NA")) {
    stop("Missing Kingdom-level information for an ASV/OTU. Remove unidentified bacteria and rerun.")
  }

  narrowest_level <- taxa_levels[[length(taxa_levels)]]
  required_levels <- setdiff(taxa_levels, narrowest_level)
  missing_levels <- setdiff(required_levels, colnames(taxa.table))
  if (length(missing_levels) > 0) {
    stop("The tax_table is missing required taxa_levels: ",
  ...
  for (lvl in taxa_levels) {
    if (lvl %in% colnames(taxa.table)) {
      ...
      col <- ifelse(is.na(col), NA_character_, paste0(prefix, col))
      col <- ifelse(col == paste0(prefix, "NA"), NA_character_, col)
      taxa.table[[lvl]] <- col
    }
  }
  ...
  fill_counts <- stats::setNames(integer(length(taxa_levels)), taxa_levels)
  for (lvl in taxa_levels) {
    if (lvl %in% colnames(taxa.table)) {
      prefix <- paste0(tolower(substr(lvl, 1, 1)), "__")
      fill_idx <- which(is.na(taxa.table[[lvl]]) |
                          !startsWith(taxa.table[[lvl]], prefix))
